...
import tech.dev.eVoyageBackend.dao.repository.customize._FinancialReportsRepository;
...
public interface FinancialReportsRepository extends JpaRepository<FinancialReports, Integer>, _FinancialReportsRepository {
	...
	/**
	 * Finds FinancialReports by using status as a search criteria.
	 * 
	 * @param status
	 * @return An Object FinancialReports whose status is equals to the given status. If
	 *         no FinancialReports is found, this method returns null.
	 */
	@Query("select e from FinancialReports e where e.status = :status and e.isDeleted = :isDeleted")
	List<FinancialReports> findByStatus(@Param("status")String status, @Param("isDeleted")Boolean isDeleted);
	/**
	 ...
	 * @throws Exception
	 */
	default String getWhereExpression(Request<FinancialReportsDto> request, HashMap<String, Object> param, Locale locale) throws Exception {
		// main query
	 ...
	 * @throws Exception
	 */
	default String generateCriteria(FinancialReportsDto dto, HashMap<String, Object> param, Integer index,  Locale locale) throws Exception{
