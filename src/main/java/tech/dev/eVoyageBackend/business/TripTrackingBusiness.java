	...
	public Response<TripTrackingDto> create(Request<TripTrackingDto> request, Locale locale)  throws ParseException {
		...
		for (TripTrackingDto dto : request.getDatas()) {
			...
			if (dto.getDepartId() != null && dto.getDepartId() > 0){
				existingDeparts = departsRepository.findOne(dto.getDepartId(), false);
				if (existingDeparts == null) {
					response.setStatus(functionalError.DATA_NOT_EXIST("departs departId -> " + dto.getDepartId(), locale));
					response.setHasError(true);
					return response;
				}
			}
				TripTracking entityToSave = null;
			entityToSave = TripTrackingTransformer.INSTANCE.toEntity(dto, existingCompanies, existingDeparts);
