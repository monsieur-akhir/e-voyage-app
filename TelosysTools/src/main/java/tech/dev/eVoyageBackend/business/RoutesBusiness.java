...
public class RoutesBusiness implements IBasicBusiness<Request<RoutesDto>, Response<RoutesDto>> {
	...
	@Autowired
	private CitiesRepository cities2Repository;
	@Autowired
	...
	public Response<RoutesDto> create(Request<RoutesDto> request, Locale locale)  throws ParseException {
		...
		for (RoutesDto dto : request.getDatas()) {
			...
			fieldsToVerify.put("originCityId", dto.getOriginCityId());
			fieldsToVerify.put("destinationCityId", dto.getDestinationCityId());
			...
			if (dto.getOriginCityId() != null && dto.getOriginCityId() > 0){
				existingCities = citiesRepository.findOne(dto.getOriginCityId());
				if (existingCities == null) {
					response.setStatus(functionalError.DATA_NOT_EXIST("cities originCityId -> " + dto.getOriginCityId(), locale));
					response.setHasError(true);
					return response;
				}
			}
			...
			// Verify if cities2 exist
			Cities existingCities2 = null;
			if (dto.getDestinationCityId() != null && dto.getDestinationCityId() > 0){
				existingCities2 = cities2Repository.findOne(dto.getDestinationCityId());
				if (existingCities2 == null) {
					response.setStatus(functionalError.DATA_NOT_EXIST("cities2 destinationCityId -> " + dto.getDestinationCityId(), locale));
					response.setHasError(true);
					return response;
				}
			}
				Routes entityToSave = null;
			entityToSave = RoutesTransformer.INSTANCE.toEntity(dto, existingCities, existingCompanies, existingCities2);
	...
	public Response<RoutesDto> update(Request<RoutesDto> request, Locale locale)  throws ParseException {
		...
		for (RoutesDto dto : request.getDatas()) {
			...
			// Verify if cities exist
			if (dto.getOriginCityId() != null && dto.getOriginCityId() > 0){
				Cities existingCities = citiesRepository.findOne(dto.getOriginCityId());
				if (existingCities == null) {
					response.setStatus(functionalError.DATA_NOT_EXIST("cities originCityId -> " + dto.getOriginCityId(), locale));
					response.setHasError(true);
					return response;
			...
			// Verify if cities2 exist
			if (dto.getDestinationCityId() != null && dto.getDestinationCityId() > 0){
				Cities existingCities2 = cities2Repository.findOne(dto.getDestinationCityId());
				if (existingCities2 == null) {
					response.setStatus(functionalError.DATA_NOT_EXIST("cities2 destinationCityId -> " + dto.getDestinationCityId(), locale));
					response.setHasError(true);
					return response;
				}
				entityToSave.setCities2(existingCities2);
			}
	...
	public Response<RoutesDto> delete(Request<RoutesDto> request, Locale locale)  {
		...
		for (RoutesDto dto : request.getDatas()) {
			...
			// tripTracking
			List<TripTracking> listOfTripTracking = tripTrackingRepository.findByRouteId(existingEntity.getId());
			...
			// travelSchedules
			List<TravelSchedules> listOfTravelSchedules = travelSchedulesRepository.findByRouteId(existingEntity.getId());
