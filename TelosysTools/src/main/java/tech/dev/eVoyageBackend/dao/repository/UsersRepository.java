...
import tech.dev.eVoyageBackend.dao.repository.customize._UsersRepository;
...
public interface UsersRepository extends JpaRepository<Users, Integer>, _UsersRepository {
	...
	/**
	 * Finds Users by using status as a search criteria.
	 * 
	 * @param status
	 * @return An Object Users whose status is equals to the given status. If
	 *         no Users is found, this method returns null.
	 */
	@Query("select e from Users e where e.status = :status and e.isDeleted = :isDeleted")
	List<Users> findByStatus(@Param("status")String status, @Param("isDeleted")Boolean isDeleted);
	/**
	...
	/**
	 * Finds Users by using companyId as a search criteria.
	 * 
	 * @param companyId
	 * @return An Object Users whose companyId is equals to the given companyId. If
	 *         no Users is found, this method returns null.
	 */
	@Query("select e from Users e where e.companies.id = :companyId and e.isDeleted = :isDeleted")
	List<Users> findByCompanyId(@Param("companyId")Integer companyId, @Param("isDeleted")Boolean isDeleted);
