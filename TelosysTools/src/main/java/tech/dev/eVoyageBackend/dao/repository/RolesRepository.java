...
import tech.dev.eVoyageBackend.dao.repository.customize._RolesRepository;
...
public interface RolesRepository extends JpaRepository<Roles, Integer>, _RolesRepository {
	...
	/**
	 * Finds Roles by using status as a search criteria.
	 * 
	 * @param status
	 * @return An Object Roles whose status is equals to the given status. If
	 *         no Roles is found, this method returns null.
	 */
	@Query("select e from Roles e where e.status = :status and e.isDeleted = :isDeleted")
	List<Roles> findByStatus(@Param("status")String status, @Param("isDeleted")Boolean isDeleted);
	/**
