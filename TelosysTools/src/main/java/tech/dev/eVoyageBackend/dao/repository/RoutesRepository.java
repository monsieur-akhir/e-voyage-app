...
public interface RoutesRepository extends JpaRepository<Routes, Integer>, _RoutesRepository {
	...
	default String generateCriteria(RoutesDto dto, HashMap<String, java.lang.Object> param, Integer index,  Locale locale) throws Exception{
		...
		if (dto != null) {
			...
			if (Utilities.notBlank(dto.getCitiesCode())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("citiesCode", dto.getCitiesCode(), "e.cities.code", "String", dto.getCitiesCodeParam(), param, index, locale));
			}
			...
			if (Utilities.notBlank(dto.getCitiesName())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("citiesName", dto.getCitiesName(), "e.cities2.name", "String", dto.getCitiesNameParam(), param, index, locale));
			}
			if (Utilities.notBlank(dto.getCitiesCode())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("citiesCode", dto.getCitiesCode(), "e.cities2.code", "String", dto.getCitiesCodeParam(), param, index, locale));
			}
