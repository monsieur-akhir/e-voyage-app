...
import tech.synelia.eVoyageBackend.dao.repository.customize._BookingsRepository;
...
public interface BookingsRepository extends JpaRepository<Bookings, Integer>, _BookingsRepository {
	...
	/**
	 * Finds Bookings by using routeId as a search criteria.
	 * 
	 * @param routeId
	 * @return An Object Bookings whose routeId is equals to the given routeId. If
	 *         no Bookings is found, this method returns null.
	 */
	@Query("select e from Bookings e where e.routes.id = :routeId and e.isDeleted = :isDeleted")
	List<Bookings> findByRouteId(@Param("routeId")Integer routeId, @Param("isDeleted")Boolean isDeleted);
